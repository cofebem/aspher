...
// R1: Task 2 replaces these stubs with the compressed slot-blocked apply.
// The declarations stay in the header (do not remove them) so the public
// interface is stable across both tasks.
void StencilPreconditioner::apply_into_blocked(
    const Eigen::VectorXd&, const std::vector<std::uint8_t>&,
    const StencilBlockLayout&, Eigen::VectorXd&) const {
    throw std::logic_error(
        "StencilPreconditioner: blocked apply not implemented (Task 2)");
}

void StencilPreconditioner::apply_single_into_blocked(
    const Eigen::VectorXf&, const std::vector<std::uint8_t>&,
    const StencilBlockLayout&, Eigen::VectorXf&) const {
    throw std::logic_error(
        "StencilPreconditioner: blocked apply not implemented (Task 2)");
}
