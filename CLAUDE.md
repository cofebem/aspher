...
| **Occupancy gate — implemented and default** (`active_occupancy_max=0.4`) | A level is restricted only when the level *below* reported a contact fraction under the threshold; the cascade already measures occupancy, so nothing must be known a priori. Applies to the finest level too (at 99% contact even finest-only restriction was 3.7× slower). `active_all_levels` now defaults **true**. Measured at Ns=1024 rough with the gate on: **2.26× / 1.35× / 1.01× / 1.00×** faster than standard at 0.11% / 11% / 47% / 99% occupancy — all the upside, no downside. Levels whose successor the gate will close no longer keep their gap field either (that alone was a 19% penalty at 47%) |
