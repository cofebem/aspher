...
- **Stagnation guard** (`solve_contact_impl`): if the complementarity error plateaus for 200 iterations (float noise floor), stop and report `converged` instead of spinning to `max_iter`. Double reaches `tol` monotonically first, so its validated iteration counts are unchanged. This is why single-precision solves may report ~200 iters at the finest level (they hit the float floor, e.g. Ns=1024 → 200 it, area still correct).
- **mallopt in `solve_contact_nested`** (`M_MMAP_THRESHOLD`/`M_TRIM_THRESHOLD` = 128 KB): the matvec + FFT preconditioner allocate large temporaries every iteration; without this glibc retains the freed blocks and **peak RSS climbs steadily over the iterations → OOM at large Ns**. With it, RSS is flat (Ns=1024 over 200 iters peaks at 0.10 GiB; ~107 B/DOF, independent of iteration count). Same remedy as the H-matrix path (commit 914b7e2).

**Large-grid memory recipe** (memory-bound nodes, e.g. Ns=16384, N≈2.7×10⁸ on 32 GiB): `hc.solve_nested(Ns, gap, p_bar, coarsest=64, q=4, leaf_side=16, precond=True, single_precision=True, light_result=True)` → solve ≈ 100 B/DOF ≈ 27 GiB. On the Python side the surface generation (meshgrid + complex FFT temporaries) is often the real hog — build it in **float32 via broadcasting** (not `np.meshgrid`), `del` temporaries, and `ctypes.CDLL("libc.so.6").malloc_trim(0)` before the solve. See `example_rough_contact.py` (its `Ns==16384` branch). The FFT preconditioner (`fourier_precond.cpp` `fft2_*`) is now **OpenMP-parallel** and transforms in place (both passes across all cores; no extra Ns×Ns temporaries): ~5× faster at Ns=2048 (1179→310 ms/iter, 20 threads) and ~2 GiB less transient at 16384, so 16384 single-precision is ~22–25 s/iter (≈30–35 min total) rather than ~an hour. **Run it alone** — the killer at 16384 is co-tenancy (peak ≈23–24 GiB of 31, so any other heavy process OOMs it). If it is still tight, preallocate the matvec/FFT buffers as reusable scratch. The hand-rolled transform only scales ~3.5× on 20 cores (memory-bandwidth bound); a real 2-D FFT lib (pocketfft/FFTW, multithreaded r2c plan) would win another ~2–3×.
...
- **FFT preconditioner speed** (partly done): `fft2_fwd/inv` are now OpenMP-parallel with in-place column passes (~5× at Ns=2048). Remaining: they still copy through per-row `std::vector` and scale only ~3.5× on 20 cores (memory-bandwidth bound). A real 2-D FFT (pocketfft/FFTW, multithreaded real-input r2c plan) would win another ~2–3× — a build-dependency change.
- **Preallocate solve buffers** (guarantees the memory fits): the matvec (M/L/y) and preconditioner (R/C/Z) temporaries are allocated per iteration; the `mallopt` in `solve_contact_nested` keeps RSS flat, but preallocating them as reusable scratch would remove the reliance on glibc behaviour and shave the peak — needed if Ns=16384 is still tight on 33 GiB.
