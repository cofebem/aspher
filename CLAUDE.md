...
- **H2 follow-ups**: active-domain/masking sparsity — **validated GO by the 2026-07-10 prototype study** (`experiments/active_set_proto.py`, `experiments/active_set_results.md`): restricted PK + gap-proximity candidate sets (δ≈0.03–0.1·rms; ~30% of fine contact islands have no coarse precursor, so gap thresholds are mandatory) recover the exact solution in 1 verification round; keep the |q| precond (restriction does not improve conditioning); projected 5–20× time / 3–8× memory at Ns=16384. C++ phase: masked H2 matvec via per-level occupancy bitmaps (measure the masked-matvec constant first), then an active-set PK driver on the nested cascade. Also: rectangular grids (nx≠ny); leaf/q auto-tuning; PCG convergence + timing sweep of H2 at Ns≥1024.
