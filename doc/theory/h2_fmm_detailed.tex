...
residual is \emph{real}: it runs as \textbf{FFTW3} 2-D real-to-complex /
complex-to-real plans, storing only the $n_h=N_s/2+1$ non-redundant $k_x$
modes (the rest follow from Hermitian symmetry); the real, even symbol
$\abs{\mathbf k}$ is applied on the half-spectrum panel, with FFTW's
unnormalised $N_s^2$ round-trip factor folded into the stored symbol. This
halves the FFT work relative to full complex transforms and keeps the
transform scratch at ${\approx}2N$ reals ($N_s^2$ real $+\;n_hN_s$ complex).
The plans (per precision, \code{FFTW\_ESTIMATE} by default; measured planning
via \code{HMC\_FFTW\_MEASURE} does not pay for single solves) and the scratch
are owned by the \code{FourierPreconditioner} object and reused across CG
iterations, and \code{apply\_into} writes into a caller-owned output buffer,
so a preconditioner application performs no heap allocation in steady state;
the mask scatter/gather passes are OpenMP-parallel and the transforms use
FFTW's internal threading. The finite-precision aspects (double accumulation
of the reductions, the cancellation-safe line-search denominator) are
documented in the companion reference \code{doc/theory/pcg.tex},
\S``Spectral Preconditioning and Finite-Precision Implementation''.
